#!/usr/bin/env python3
"""Structured Memory + Dual-Route Reasoning (SM2) paired experiment scheduler.

Runs the SAME suite/tasks/seeds/turns as SM1, with ``RPENT_STRUCTURED_MEMORY=1
RPENT_DUAL_ROUTE=1`` (harness keeps Structured Memory, plus the Fast/Slow
decider: Fast steps execute conservative zero-arg actions without an LLM call).
Control = the SM1 structured arm reused from analysis/structured_memory_runs.csv
(paired by task/seed), exactly as SM1 reused the progress-gate baseline.

Also runs 2 fixed-code structured spot-checks (t7s2 / t0s5, gate-on only) to
confirm Fix A (R8 counter scoping) causes no behavioural drift.

One phase (SM2): dual_route on libero_spatial_task t0/t7/t9, seeds 1-10,
repeat 1 -> 30 runs. 8 workers (4 GPUs x 2) — unlimited API supply. Writes
analysis/dual_route_runs.csv (Fix B: explicit RUN_FIELDNAMES).

Usage: nohup python scripts/dual_route_exp.py >> .gap_run/dr_super.log 2>&1 &
"""
...
SM1_RUNS_CSV = os.path.join(ROOT, "analysis", "structured_memory_runs.csv")
STATUS_MD = os.path.join(LOGS_DIR, "dr_status.md")
...
INFRA_PAUSE_S = 600
RUNTIME_S = 4500
WORKERS_PER_GPU = int(os.environ.get("DR_WORKERS_PER_GPU", "2"))
# Dev-machine safety cap (2026-09-05): shared dev box (cgroup 200G RAM / 32-core
# quota); total workers default <= 8. Override with DR_MAX_WORKERS / DEV_MAX_WORKERS
# only when resources allow (dedicated node).
MAX_WORKERS = int(os.environ.get("DR_MAX_WORKERS",
                                 os.environ.get("DEV_MAX_WORKERS", "8")))
GPU_IDLE_MB = int(os.environ.get("DR_GPU_IDLE_MB", "4000"))
STATUS_INTERVAL = 300

# Same structured gate as SM1, plus the dual-route switch. Spot-checks run the
# plain structured gate (no dual-route) to isolate Fix A drift.
COND_ENV = {
    "dual_route": {"RPENT_STRUCTURED_MEMORY": "1", "RPENT_DUAL_ROUTE": "1"},
    "structured": {"RPENT_STRUCTURED_MEMORY": "1"},
}
P0_TASKS = [0, 7, 9]
P0_SUITE = "libero_spatial_task"
# fixed-code structured spot-checks (confirm Fix A has no behavioural drift)
SPOT_CHECKS = [(7, 2), (0, 5)]

# Fix B: explicit header — reused control rows (SM1 metric cols were dropped by
# the old buggy header) can never silently lose columns here.
RUN_FIELDNAMES = [
...
def run_episode(ep, gpu):
    ...
    outdir = os.path.join(DR_DIR, f"{ts}_{cond}_{suite}_t{task}_s{seed}_r{repeat}")
...
def record_existing_control(state):
    """Reuse SM1 structured repeat-1 rows (same suite/tasks/seeds) as the
    paired control, then add fresh structured spot-checks."""
    done = completed_keys()
    if not os.path.exists(SM1_RUNS_CSV):
        log("structured_memory_runs.csv missing; cannot reuse control rows")
        return
    for r in csv.DictReader(open(SM1_RUNS_CSV)):
        if (r["cond"] != "structured" or r["stage"] != "SM1"
                or r["suite"] != P0_SUITE
                or int(r["task"]) not in P0_TASKS
                or int(r["repeat"]) != 1):
            continue
        k = (STAGE, P0_SUITE, int(r["task"]), int(r["seed"]), "structured", 1)
        if k in done:
            continue
        # dir fields come from the SM1 row; result column is authoritative for
        # the control arm (dir still analysed live in the report).
        res = dict(ts=r["ts"], dir=r["dir"], rc=int(r["rc"] or 0),
                   wall_s=r.get("wall_s") or "", result=r["result"])
        res.update(metric_fields(r["dir"]))  # live structured_metrics.json
        append_run(state, dict(stage=STAGE, suite=P0_SUITE,
                               task=int(r["task"]), seed=int(r["seed"]),
                               cond="structured", repeat=1), res)
        log(f"reused control {P0_SUITE} t{r['task']} s{r['seed']} -> {r['result']}")
...
def build_episodes(done):
    ...
    for t in P0_TASKS:
        for sd in range(1, N_EVAL + 1):
            k = (STAGE, P0_SUITE, t, sd, "dual_route", 1)
            if k not in done:
                eps.append(dict(stage=STAGE, suite=P0_SUITE, task=t, seed=sd,
                                cond="dual_route", repeat=1))
    for t, sd in SPOT_CHECKS:
        k = (STAGE, P0_SUITE, t, sd, "structured", 2)
        if k not in done:
            eps.append(dict(stage=STAGE, suite=P0_SUITE, task=t, seed=sd,
                            cond="structured", repeat=2))
    return eps
...
def write_status():
    ...
    lines = [
        "# Structured Memory + Dual-Route (SM2) 实验状态\n",
...
def main():
    pg.preflight(label="dual_route", lock_name="dual_route")
    pg.ensure_egl()
    os.makedirs(DR_DIR, exist_ok=True)
    state = load_state()
    log(f"=== Structured Memory + Dual-Route scheduler start | workers/gpu={WORKERS_PER_GPU} "
